...
function renderAnalysis() {
  ...
  if (analysis.mindset) {
    ...
    if (mso.violations && mso.violations.length > 0) {
      mso.violations.forEach(function(v) {
        html += '<div style="font-size:10px;color:var(--red);padding:3px 0;line-height:1.4;">\u26A0 <strong>' + v.rule + '</strong> \u2014 ' + v.detail.substring(0, 120) + (v.detail.length > 120 ? '...' : '') + '</div>';
      });
    }
    if (mso.wins && mso.wins.length > 0) {
      html += '<div style="margin-top:6px;">';
      mso.wins.slice(0, 2).forEach(function(w) {
        html += '<div style="font-size:10px;color:var(--green);padding:2px 0;line-height:1.4;">\u2713 ' + w.substring(0, 100) + (w.length > 100 ? '...' : '') + '</div>';
      });
      html += '</div>';
    }
    html += '</div>';
  ...
  if (analysis.probabilityMap && analysis.probabilityMap.length > 0) {
    html += '<div style="font-size:10px;color:var(--text-muted);margin-bottom:12px;padding:8px 12px;background:var(--bg-secondary);border-radius:8px;line-height:1.5;">Ranked by probability of a 3%+ move tomorrow. Based on multi-day patterns, catalyst proximity, IV levels, technical setup, and sector correlation.</div>';
    html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">';
  ...
  if (analysis.tomorrowWatch) {
    ...
    html += '<div style="font-size:10px;font-weight:800;color:var(--blue);margin-bottom:10px;text-transform:uppercase;letter-spacing:0.08em;">\u{1F52E} Tomorrow\'s Playbook</div>';
    html += '<div style="font-size:10px;color:var(--text-secondary);line-height:1.7;white-space:pre-wrap;">' + analysis.tomorrowWatch + '</div>';
    html += '</div>';
  }
  html += '</div>';
  ...
  if (analysis.mindset) {
    ...
    if (ms.violations && ms.violations.length > 0) {
      ms.violations.forEach(function(v) {
        html += '<div style="font-size:10px;color:var(--text-secondary);padding:6px 0;border-bottom:1px solid var(--border);line-height:1.5;"><strong style="color:var(--red);">' + v.rule + '</strong><br>' + v.detail + '</div>';
      });
    } else {
    ...
    if (ms.wins && ms.wins.length > 0) {
      ms.wins.forEach(function(w) {
        html += '<div style="font-size:10px;color:var(--text-secondary);padding:6px 0;border-bottom:1px solid var(--border);line-height:1.5;">' + w + '</div>';
      });
    }
    html += '</div>';
    html += '</div>';
...
function renderRecentEntries(parentEl) {
  ...
  if(entriesToShow.length === 0) {
    html += '<div style="padding:14px;text-align:center;color:var(--text-muted);font-size:10px;">No entries yet. Click a button above to auto-generate.</div>';
  } else {
...
function addChatMessage(role, text) {
  ...
  avatar.style.cssText = 'width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;flex-shrink:0;' +
